    ...
    if(isset($_REQUEST["lname"]) && $_REQUEST["lname"]!=""){
      if(isset($_REQUEST["email"]) && $_REQUEST["email"]!=""){
        if(isset($_REQUEST["password"]) && $_REQUEST["password"]!="" && strlen($_REQUEST["password"])>=6){
          ...
          $query="select * from `wryton_user` where `email`='{$email}'";
          ...
          if(mysqli_num_rows($result)==0){
            ...
            //send mail
            $query="INSERT INTO `wryton_user`(`fname`,`lname`,`email`,`password`,`security_key`,`verified`,`status`,`registered_on`) VALUES ('{$fname}','{$lname}', '{$email}','{$password}','{$security_key}', '{$verified}', '{$status}', '{$current_date}')";
            if(mysqli_query($con,$query)){
              
              $output='{"status":"success", "remark":"Your account is created. please login and enjoy WRYTON"}';
            }else{
